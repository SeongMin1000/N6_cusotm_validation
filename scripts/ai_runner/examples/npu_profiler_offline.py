...
def run(args):
    ...
    # =================================================================
    # [추가할 부분] 오프라인 예측치 출력 후 종료
    if c_npu_network:
        logger.info('')
        logger.info('  Offline Predicted Performance (C-Model Analysis)')
        
        total_compute = 0
        total_max = 0
        total_ops = 0
        
        for epoch in c_npu_network.epochs:
            total_ops += epoch.perfs.get("ops", 0)
            total_compute += epoch.perfs.get("compute_cycles", 0)
            total_max += epoch.perfs.get("max_cycles", 0)
            
        npu_freq = 1000000000  # 1 GHz, 실제 NPU 주파수로 대체 가능
        
        # 기존 로그와 완벽하게 동일한 띄어쓰기 폼(%-21s)으로 출력
        logger.info('   %-21s : %s (total max_cycles)', 'cycles', total_max)
        logger.info('   %-21s : %s (total compute_cycles)', 'ideal cycles', total_compute)
        logger.info('   %-21s : %s', 'ops', total_ops)
        
        if total_max > 0:
            logger.info('   %-21s : %s', 'ops/cycle', int(total_ops/total_max))
            logger.info('   %-21s : %.1f (based on estimated total max_cycles)', 'inf/s', npu_freq / total_max)
            
        if total_compute > 0:
            logger.info('   %-21s : %s', 'ideal ops/cycle', int(total_ops/total_compute))
            logger.info('   %-21s : %.1f', 'ideal inf/s', npu_freq / total_compute)
            
        logger.info('')
        return 0  # 보드 연결 없이 스크립트 정상 종료
    # =================================================================
