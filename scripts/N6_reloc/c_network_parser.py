...
from exceptions import ExceptionErr, RelocParserError
    ...
    if val:
        ...
        if base_.startswith('0x'):  # absolute base@
            ...
            if val1 or base_.startswith('(uintptr_t)'):
                ...
                raise RelocParserError(f'Unable to read the base@ from \'{base_}\'')
        return base_
    return 0
...
def parse_compiler_options(lines: Union[TextIO, List[str]],
    ...
    all_buffers_info_, model_file_, mdesc_file_, json_quant_file_, mempool_ = False, '', '', '', ''
    ec_ = False
    version_ = (0, 0, 0, 0)
    c_name_ = ''
    for line in lines:
        if 'GIT_DESCRIPTION' in line:
            ...
            file_path_ = _get_last_item(line).replace('\\', '::').replace('/', '::')
            mdesc_file_ = file_path_.split('::')[-1]
    ...
    options = CompilerDesc(desc_, sha_, date_,
                           ...
                           mempool_, mdesc_file_, c_name_, ec_, version_)
...
def parse_mempools(lines: Union[TextIO, List[str]],
    ...
    if len(pool_id_sizes_) != len(pool_descs_):
        raise RelocParserError('Number of pools and descriptors are not aligned')
    ...
    if len(mempool_c_labels_) != nb_mempool_relative:
        raise RelocParserError('Number of mempool relative and c-label are not aligned')
    ...
    for idx, line in enumerate(lines):
        ...
        if not line_descs:
            ...
            if name_ in func_descriptors:
                raise RelocParserError("Start/Stop function %s already parsed" % name_)
...
def parse_epoch_block_list(lines: Union[TextIO, List[str]],
    ...
    if len(start_stop_funcs):
        raise RelocParserError("%s func(s) not associated to a epoch descriptors" % len(start_stop_funcs))
...
def parse_c_network_file(params):
...
def main():
    ...
    parser.add_argument('--input', '-i', metavar='STR', type=str, help='generated c-file',
