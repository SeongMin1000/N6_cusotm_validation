...
void aiPbCmdNNRun(const reqMsg *req, respMsg *resp, void *param)
{
  ...
  // /* Loop through all NPU networks */
  // for (int net_idx = 0; net_idx < NPU_NETWORK_NUMBER; net_idx++) {
  //   ctx = aiExecCtx("", net_idx);  /* Use empty name to select by index */
  //   if (!ctx) {
  //     continue;  /* Skip if this network is not initialized */
  //   }
